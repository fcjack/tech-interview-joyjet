...
public class CartServiceTest {
    ...
    public void testCheckoutWithDiscounts() {
        ...
        Discount discount = new Discount(1, Discount.DiscountType.PERCENTAGE, (double) 10);
        Discount discount2 = new Discount(3, Discount.DiscountType.AMOUNT, (double) 2300);

        InputData inputData = new InputData();
        inputData.setArticles(Arrays.asList(article, article2, article3, article4));
        inputData.setDiscounts(Arrays.asList(discount, discount2));
        inputData.setCarts(Collections.singletonList(cart));
        cartService.calculate(inputData);

        Assert.assertEquals(125_000, cart.getTotal().longValue());
    }
    ...
    @Test
    public void testWithDeliveryFeesAndDiscounts() {
        cartService = new CartServiceImpl();
        InputData inputData = new InputData();
        inputData.setArticles(Arrays.asList(article, article2, article3, article4));
        ...
        Assert.assertEquals(125000, cart.getTotal().longValue());
        Assert.assertEquals(207000, cart2.getTotal().longValue());
    }
